...
namespace fqhex
{
    ...
    private:
        ...
        static sub_if_result sub_if(CharT min, CharT max, __m128i x, CharT value) {
            auto mask_min = _mm_cmpgt_epi32(x, _mm_set1_epi32(min - 1));
            auto mask_max = _mm_cmplt_epi32(x, _mm_set1_epi32(max + 1));
            auto mask = _mm_and_si128(mask_min, mask_max);
            auto mask_value = _mm_and_si128(mask, _mm_set1_epi32(value));
            return { _mm_sub_epi32(x, mask_value), mask };
        }
        ...
        static __m128i to_string(__m128i x) {
            ...
            auto hi = _mm_and_si128(_mm_slli_si128(ex, 4), _mm_set1_epi8(0x0f));
            auto v = _mm_or_si128(hi, lo);

            auto mask = _mm_cmpgt_epi32(v, _mm_set1_epi32(9));
            auto mask_0x27 = _mm_and_si128(mask, _mm_set1_epi32(0x27));
            auto add_0x30 = _mm_add_epi32(v, _mm_set1_epi32(0x30));
            auto s = _mm_add_epi32(add_0x30, mask_0x27);

            return s;
        }

        static void to_bytes_16(std::span<const CharT> in, std::span<ByteT> out) {
            auto x = _mm_loadu_si128(reinterpret_cast<const __m128i*>(in.data()));
            _mm_storeu_si16(out.data(), to_bytes(x));
        }

        static void to_string_16(std::span<const ByteT> in, std::span<CharT> out) {
            auto x = _mm_loadu_si16(in.data());
            auto s = to_string(x);
    ...
    public:
        static void to_bytes(std::span<const CharT> in, std::span<ByteT> out) {
            ...
            for (size_t i = 0; i < count; i++)
                to_bytes_16(in.subspan(i * 4), out.subspan(i * 2));

            if (in.size() % 4 >= 2)
                hex_generic<CharT, ByteT>::to_bytes(in.subspan(count * 4), out.subspan(count * 2));
        }
        ...
        static constexpr void to_string(std::span<const ByteT> in, std::span<CharT> out) {
            ...
            for (size_t i = 0; i < count; i++)
                to_string_16(in.subspan(i * 2), out.subspan(i * 4));

            if (in.size() % 8 > 0)
                hex_generic<CharT, ByteT>::to_string(in.subspan(count * 2), out.subspan(count * 4));
        }
    };
}
